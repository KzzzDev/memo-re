# <font color="orange">悲しいお知らせ</font>

本番環境にデプロイした環境が停止されました。<br>
経緯を簡単に説明すると、学校側で用意されたAzure のリソースを使用していたのですが、本番環境での運用開始３日目にして使用できるリソースの上限に達しました。<br>
生徒側には使用できる上限がわからず、学校側にコスト管理は任せざるを得なかった結果がこうなってしまい非常に残念です（ ;  ; ）
